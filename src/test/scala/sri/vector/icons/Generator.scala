...
object Generator {
  ...
  def getScalaName(in: String) = {
    val in1 = in.replace("\"", "")
    if (Try(in1.head.toString.toInt).isSuccess) s"_$in1"
    else in1
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    def getScalaName(in: String) = {
      val in1 = in.replace("\"", "").replace("-", "_").toUpperCase
      if (Try(in1.head.toString.toInt).isSuccess) s"_$in1"
      else in1
    }
    ...
    val result = in
      .split(",")
      .filter(_.contains(":"))
      .map(l => l.trim.split(":").head)
      .map(n =>
        s"@inline def ${getScalaName(n)} = $n.asInstanceOf[MaterialIconsName]")
      .mkString("\n")
